...
void Writeset_trx_dependency_tracker::get_dependency(THD *thd,
  ...
  */
  bool can_use_writesets =
      ...
      // 写集合不等于0，意思是存在写集合，则表示是dml语句并且实际更新了行
      // 事务引用的表中存在没有key的情况(主键、非空列唯一索引。这个理解可能不准确，难道有key的话，就不能使用写集合了做依赖追踪？)
      // 空事务
      ...
      // rows in history
      //需要确保此事务的writeset与writeset_history使用相同的hash算法
      ...
      // 事务的writeset没有超出writeset_size大小，如果这个条件不满足，那就不使用writeset_history计算父事务，并且要清空writeset_history
  ...
  if (can_use_writesets) {
    ...
    */
    ...
    for (std::vector<uint64>::iterator it = writeset->begin();
      ...
      if (hst != m_writeset_history.end()) { //如果存在冲突的行
        //hst是一个{row_hash:unsigned int64: transaction.sequence_number}结构，
        // 简而言之，writeset_history中的每一行，都对应了更新这个行的最新的事务的sequence_number值，
        // 当下一次有新提交的事务的writeset包含这个行的时候，用以计算新提交的事务的父事务
        ...
        if (!exceeds_capacity)
          //如果没有超过writeset_size大小，并且没有行冲突，则把这行添加到writeset_history中
    ...
    */
    if (!write_set_ctx->get_has_missing_keys()) { //对于事务更新的表中不存在缺失key(主键或者非空唯一性索引)的情况，
      //取其根据writeset计算出来的父事务的sequence_number和组提交父事务的sequence_number的最小值，保持最大可能的并行性
      /*
  ...
  // 如果超过m_opt_max_history_size，则需要清空writeset_history，
  // 并且将m_writeset_history_start更新为当前事务的sequence_number，
  // 下一次当新事务提交，因为writeset_history是空的，m_writeset_history_start就可以作为新事务的初始父事务sq
