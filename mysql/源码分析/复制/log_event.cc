...
/**
   ...
   g-event: 包含分区信息的小型组（例如，table_map语句）
...
Slave_worker *Log_event::get_slave_worker(Relay_log_info *rli) {
  ...
  /*
  if  <s> 
         ("BEGIN"或者"XID START" event) 
         || (GTID或者匿名GTID event)
         || <b>
                (当前event组还没有读取到 "BEGIN"或者"XID START" event ) 
             && (当前组还没有读取到GTID或者匿名GTID event) 
             && (
                      (协调器的GAQ是空的：没有正在处理的事务/event group)
                  ||  (最近放入GAQ的事务，已经分配了worker处理event重放。有一些事务不会经历BEGIN和GTID事件，比如Previous_gtids_log_event。
                       这里可能表明，上一个group已经读取结束，开始读取下一个group了。
                       一个事务、event如何在这些结构中流转，还需要总结一下)
                )
            <eb>
      <es>
  */
      ...
      */
      ...
      if (is_s_event || is_gtid_event(this)) {
        ...
        if (starts_group()) {
          ...
          // curr_group_seen_begin在处理"BEGIN"或者"XID START" event 时设置为true
